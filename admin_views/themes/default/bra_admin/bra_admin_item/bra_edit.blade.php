                                ...
                                <div class="layui-colla-content layui-show">
                                    <fieldset>
                                        ...
                                        @foreach($field_list as $k=>$field)
											<?php
											if (empty($field['form_str']) || empty(trim($field['form_str']))) {
												continue;
											}
											?>
                                            @if($field['field_group'] == $label['id'])

                                                <div class="field has-addons" id="row_{{$k}}">
                                                    @if (!empty($field['sub_fields']))
                                                        @foreach ($field['sub_fields'] as $sub_field)
															<?php
															if (empty($sub_field['form_str']) || empty(trim($sub_field['form_str']))) {
																continue;
															}
															?>
                                                            <div class="layui-inline {$sub_field.field_group_class}">
                                                                <label class="layui-form-label" style="width:98px">{{ trans($sub_field['slug'])}}
                                                    ...
                                                    @else
                                                        <div class="layui-block">
                                                            <label class="layui-form-label" style="width:98px">{{ trans($field['slug'])}}
                                                                @if($sub_field['must_fill']) <span class="ui red">*</span> @endif</label>
                        ...
                        <div class="field" id="row_{{$k}}">
                            ...
                            @else
                                <div class="field has-addons">
                                    <div class="control">
                                        <div class="bra-btn" style="width:160px">
                                            {{ trans($field['slug'] ?? '')}}

                                            @if($field['must_fill']) <span
