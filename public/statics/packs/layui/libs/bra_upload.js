    ...
    var bra_upload = {
        ...
        init_mutil_wx_upload: function (field_name, number) {
            var wx_choose = {
                ...
                choose: function () {
                    var that = this;
                    require(['wx', 'bra_pay'], function (wx, bra_pay) {
                        bra_pay.get_sign(location.href, function (jssdk_obj) {
                            wx.config(jssdk_obj);
                            wx.ready(function () {
                                wx.chooseImage({
                                    count: 5, // 默认9
                ...
                upload: function (wx , i) {
                    ...
                    wx.uploadImage({
                        ...
                        success: function (res) {
                            console.log(res);
                            var download_api = "/wechat/bra_auth/download_media";
                            // download media
                            layui.use(['layer'], function () {
                                ...
                                $.get(
                                    download_api, {
                                        ...
                                        var bra_annex = $('<div id="' + field_name + '_' + i + '_img" class="layui-upload-img  weui-uploader__file"><img src=\"' + res.url + '\" alt=\"' + '\" class=\"layui-upload-img\"><input type="hidden" value="' + res.data.id + '" name="' + $real_form_name + '"><i class=\'icon close\'></i></div>');
                                        bra_annex.find('.close').on('click', function () {
                                            bra_annex.remove();
                                        });
                                        $('#' + field_name).append(bra_annex);
                                    }, 'json'
                                );
                            });
