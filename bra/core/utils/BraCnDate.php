...
//debugger;
	...
	/**
	 * 计算2个阳历日期之间的天数
	 * @param $year string 阳历年
	 * @param $cmonth
	 * @param $cdate
	 * @param $dmonth string 阴历正月对应的阳历月份
	 * @param $ddate string 阴历初一对应的阳历天数
	 * @return float
	 */
...
//echo __FILE__.'->'.__LINE__.' $day1='.$day1,',$day2='.$day2.'<br/>'.chr(10);
		$data = array();
		if ($idx1 === 0) {
			$data['name'] = $term_name[count($term_name) - 1] . '后';
		} else {
			if ($day < $day1) {
				$data['name'] = $term_name[$idx1 - 1] . '后';
			} else if ($day == $day1) {
				$data['name'] = $term_name[$idx1];
			} else if ($day > $day1 && $day < $day2) {
				$data['name'] = $term_name[$idx1] . '后';
			} else if ($day == $day2) {
				$data['name'] = $term_name[$idx1 + 1];
			} else if ($day > $day2) {
				$data['name'] = $term_name[$idx1 + 1] . '后';
			}
		}
